...
VERSION=0.0.1; echo " ongoing scrna pipeline version $VERSION"
#last updated version 2022-06-09
# test on 
...
if ! options=$(getopt --name pipeline --alternative --unquoted --options hs:d:t:m:vw:f:S:c:a:x: --longoptions sample:,dir:,steps:,nFRNAl:,nFRNAu:,main:,inte:,pmt:,msd:,verbose,help -- "$@")
then
...
if [[ -z $STEP7m ]]; then  STEP7m="T"; fi
if [[ -z $STEP7i ]]; then  STEP7i="T"; fi
